...
class PurchasesServices:
    ...
    @staticmethod
    def get_completed_purchase_service(
        user_id: str, purchase_id: str
    ) -> dict[str, Any] | None:

        completed_purchase = PurchasesRepository.get_completed_purchase(
            user_id, purchase_id
        )

        if completed_purchase:
            formatted_purchase = {
                "purchase_id": completed_purchase.get("purchase_id"),
                "purchase_status": completed_purchase.get("purchase_status"),
                "original_owner_id": completed_purchase.get("original_owner_id"),
                "user_id": completed_purchase.get("user_id"),
                "book_id": completed_purchase.get("book_id"),
                "title": completed_purchase.get("title", ""),
                "author": completed_purchase.get("author", ""),
                "image": completed_purchase.get("image"),
                "from": completed_purchase.get("from", ""),
                "all_fees_captured": completed_purchase.get("all_fees_captured", False),
                "reserved_at": DateUtils.format_datetime_to_iso(
                    completed_purchase.get("reserved_at")
                ),
                "reservation_expires_at": DateUtils.format_datetime_to_iso(
                ...
                "pickup_confirmation_started_at": DateUtils.format_datetime_to_iso(
                    completed_purchase.get("pickup_confirmation_started_at")
                ),
                "user_confirmed_pickup": completed_purchase.get(
                    "user_confirmed_pickup", False
                ),
                "owner_confirmed_pickup": completed_purchase.get(
                    "owner_confirmed_pickup", False
                ),
                "user_rated": completed_purchase.get("user_rated", False),
                "owner_rated": completed_purchase.get("owner_rated", False),
                "transfer_decision_pending": completed_purchase.get(
                    "transfer_decision_pending", False
                ),
                "ownership_transferred": completed_purchase.get(
                    "ownership_transferred"
                ),
                "cost": (
                    int(completed_purchase.get("cost", 0))
                    if completed_purchase.get("cost") not in (None, "")
                    else 0
                ),
                "meetup_date": (
                    DateUtils.extract_date(completed_purchase.get("meetup_date"))
                    if completed_purchase.get("meetup_date")
                    else None
                ),
            }
    ...
    @staticmethod
    def get_completed_sale_service(
        user_id: str, purchase_id: str
    ) -> dict[str, Any] | None:

        completed_sale = PurchasesRepository.get_completed_sale(user_id, purchase_id)

        if completed_sale:
            formatted_sale = {
                "purchase_id": completed_sale.get("purchase_id"),
                "purchase_status": completed_sale.get("purchase_status"),
                "original_owner_id": completed_sale.get("original_owner_id"),
                "user_id": completed_sale.get("user_id"),
                "book_id": completed_sale.get("book_id"),
                "title": completed_sale.get("title", ""),
                "author": completed_sale.get("author", ""),
                "image": completed_sale.get("image"),
                "to": completed_sale.get("to", ""),
                "all_fees_captured": completed_sale.get("all_fees_captured", False),
                "reserved_at": DateUtils.format_datetime_to_iso(
                    completed_sale.get("reserved_at")
                ),
                "reservation_expires_at": DateUtils.format_datetime_to_iso(
                ...
                "pickup_confirmation_started_at": DateUtils.format_datetime_to_iso(
                    completed_sale.get("pickup_confirmation_started_at")
                ),
                "user_confirmed_pickup": completed_sale.get(
                    "user_confirmed_pickup", False
                ),
                "owner_confirmed_pickup": completed_sale.get(
                    "owner_confirmed_pickup", False
                ),
                "user_rated": completed_sale.get("user_rated", False),
                "owner_rated": completed_sale.get("owner_rated", False),
                "transfer_decision_pending": completed_sale.get(
                    "transfer_decision_pending", False
                ),
                "ownership_transferred": completed_sale.get("ownership_transferred"),
                "cost": (
                    int(completed_sale.get("cost", 0))
                    if completed_sale.get("cost") not in (None, "")
                    else 0
                ),
                "meetup_date": (
                    DateUtils.extract_date(completed_sale.get("meetup_date"))
                    if completed_sale.get("meetup_date")
                    else None
                ),
            }
    ...
    def reject_purchase_request(
        ...
        try:
            ...
            if not purchase:
                return None, "Purchase not found", None
            ...
            if not buyer_user_id:
                return None, "Buyer user ID not found", None
            buyer_user_id_str = str(buyer_user_id)
    ...
    def confirm_pickup(
        ...
        try:
            ...
            if purchase_status != "awaiting_pickup_confirmation":
                return (
                    None,
                    f"Cannot confirm pickup. Current status: {purchase_status}",
                    None,
                    None,
                )
            ...
            if not is_owner and not is_buyer:
                return (
                    None,
                    "Unauthorized: Only the buyer or owner can confirm pickup",
                    None,
                    None,
                )
            ...
            return result, None, original_owner_id, buyer_user_id
