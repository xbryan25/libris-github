...
const userName = computed(() => {
  return props.from === 'rental' ? (props.item as Rental).from : (props.item as Lending).to;
});
    ...
    <div class="flex gap-6">
      ...
      <div class="flex-1">
        <div class="flex justify-between items-start mb-4">
          ...
          <span
            :class="[
              getStatusBadge(item.rent_status).color,
        ...
        <div class="grid grid-cols-2 gap-4 mt-6">
          <div>
            <p v-if="props.isCompleted" class="text-sm text-muted">
              {{ from === 'rental' ? 'Rented from' : 'Lent to' }}
            </p>

            <p v-else class="text-sm text-muted">
              {{ from === 'rental' ? 'Renting from' : 'Lending to' }}
            </p>
