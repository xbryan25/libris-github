...
async function sendPurchase() {
  ...
  try {
    ...
    await createPurchase({
      ...
      meetup_location: selectedAddress.value,
      latitude: selectedAddressLatitude.value as number,
      longitude: selectedAddressLongitude.value as number,
  ...
  <UModal v-model:open="isOpenPurchaseBookModal" :ui="{ content: 'max-w-lg' }" :dismissible="true">
    ...
    <template #body>
      <div class="p-4 bg-surface-hover space-y-3 rounded-sm">
        ...
        <div
          class="flex items-center justify-between font-semibold text-base"
          :class="{ 'text-red-500': hasInsufficientFunds }"
        >
          ...
          <div class="flex items-center gap-1">
            <Icon
              ...
              class="w-5 h-5"
              :class="hasInsufficientFunds ? 'text-red-500' : 'text-accent'"
            />
      ...
      >
        ...
        <span> Insufficient available funds. </span>
      </div>
      ...
      <div class="mt-4">
        ...
        <div class="flex gap-2 mt-1">
          <VueTimepicker
            ...
            @change="validateTimeWindow"
            format="HH:mm"
            :use12-hour="false"
            placeholder="Start Time"
          />
          <VueTimepicker
            v-model="meetupEndTime"
            @change="validateTimeWindow"
            format="HH:mm"
            :use12-hour="false"
