...
const { fetchUserSales } = useUserSales();
const { rejectPurchase, loading: submitting } = usePurchaseActions();
...
const handleSubmit = async () => {
  ...
  if (result.success) {
    ...
    submitError.value = result.error || 'Failed to reject purchase';
  }
};
...
<template>
  <UModal v-model:open="isOpen">
    <template #header>
      <h2 class="text-2xl font-semibold">Reject Purchase Request</h2>
    </template>

    <template #body>
      <div class="space-y-4">
        <p class="text-muted">Please provide a reason for rejecting this purchase request. This will help the buyer understand your decision.</p>
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 dark:bg-blue-900/20 dark:border-blue-800">
          <div class="flex items-start gap-2">
            ...
            <p class="text-sm text-blue-900 dark:text-blue-300">
              The purchase entry will be deleted and reserved funds will be released back to the buyer. The buyer will receive a notification with your reason for rejection.
            </p>
          </div>
        </div>
      </div>
