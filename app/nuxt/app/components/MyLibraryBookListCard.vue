<script setup lang="ts">
import type { MyLibraryBook } from '~/types';

const props = defineProps<{
  cardType: string;
  bookDetails?: MyLibraryBook | null;
}>();


</script>

<template>
  <div
    v-if="props.cardType === 'hasContent'"
    class="w-full h-[440px] flex flex-col items-center justify-center"
  >
    <div class="flex-[2] w-full rounded-t-xl overflow-hidden">
      <NuxtImg
        v-if="props.bookDetails?.firstImageUrl"
        :src="props.bookDetails?.firstImageUrl"
        class="w-full h-full object-cover"
        alt="Auth image"
      />
      <NuxtImg
        v-else
        src="/images/noImageImage.jpg"
        class="w-full h-full object-cover"
        alt="Auth image"
      />
    </div>

    <div class="flex-1 flex flex-col gap-[10px] px-5 py-4 bg-surface-hover w-full rounded-b-xl">
      <div class="flex-1 flex flex-col gap-1 items-center pb-1">
        <UTooltip :text="props.bookDetails?.title">
          <p class="text-lg text-center font-semibold text-base truncate w-full">
            {{ props.bookDetails?.title }}
          </p>
        </UTooltip>

        <UTooltip :text="props.bookDetails?.author" class="flex-[3]">
          <p class="text-left text-xs text-base truncate">by {{ props.bookDetails?.author }}</p>
        </UTooltip>
      </div>

      <div
        v-if="
          !props.bookDetails?.rentStatus ||
          props.bookDetails?.rentStatus === 'pending' ||
          props.bookDetails?.rentStatus === 'completed'
        "
        class="flex flex-col gap-[10px]"
      >
        <div class="flex gap-2 items-start">
          <UBadge
            v-if="
              ['rent', 'both'].includes(
                props.bookDetails?.availability ? props.bookDetails?.availability : '',
              )
            "
            class="font-bold rounded-full"
            >For Rent</UBadge
          >
          <UBadge
            v-if="
              ['purchase', 'both'].includes(
                props.bookDetails?.availability ? props.bookDetails?.availability : '',
              )
            "
            color="error"
            class="font-bold rounded-full"
            >For Sale</UBadge
          >
        </div>

        <div class="flex flex-col gap-1">
          <div
            v-if="
              ['rent', 'both'].includes(
                props.bookDetails?.availability ? props.bookDetails?.availability : '',
              )
            "
            class="flex gap-2 items-center"
          >
            <div class="flex-1 flex gap-1 items-center">
              <Icon name="solar:clock-circle-bold" class="w-5 h-5 text-base" />
              <p class="flex-1 text-sm font-semibold text-base">Rent</p>
            </div>

            <div class="flex items-center">
              <Icon name="fluent:book-coins-20-regular" class="w-5 h-5 text-accent" />
              <p class="text-xs text-primary font-semibold">
                {{ props.bookDetails?.dailyRentPrice }}/day
              </p>
            </div>
          </div>

          <div
            v-if="
              ['purchase', 'both'].includes(
                props.bookDetails?.availability ? props.bookDetails?.availability : '',
              )
            "
            class="flex gap-2 items-center"
          >
            <div class="flex-1 flex gap-1 items-center">
              <Icon name="solar:cart-large-2-bold-duotone" class="w-5 h-5 text-base" />
              <p class="flex-1 text-sm font-semibold text-base">Buy</p>
            </div>

            <div class="flex items-center">
              <Icon name="fluent:book-coins-20-regular" class="w-5 h-5 text-accent" />
              <p class="text-xs text-primary font-semibold">
                {{ props.bookDetails?.purchasePrice }}
              </p>
            </div>
          </div>

          <div
            v-if="
              ['rent', 'purchase'].includes(
                props.bookDetails?.availability ? props.bookDetails?.availability : '',
              )
            "
            class="flex gap-2 items-center h-5"
          />

          <div class="flex gap-2 pt-2">
            <UButton class="flex-1 justify-center cursor-pointer">
              <Icon name="material-symbols:edit" class="w-5 h-5 text-bg" />
              <p>Edit</p>
            </UButton>
            <UButton color="error" class="flex-1 justify-center cursor-pointer">
              <Icon name="material-symbols:delete-sharp" class="w-5 h-5 text-bg" />
              <p>Delete</p>
            </UButton>
          </div>
        </div>
      </div>

      <div v-else class="flex flex-col justify-end pt-[49px]">
        <USeparator color="primary" type="solid" class="" />

        <div class="flex flex-col items-start">
          <div class="flex flex-col pt-2 gap-2">
            <p>Currently rented by:</p>

            <div class="flex items-center gap-2">
              <NuxtImg
                v-if="props.bookDetails?.renterProfileImageUrl"
                :src="props.bookDetails?.renterProfileImageUrl"
                class="w-8 h-8 rounded-full object-cover"
                alt="Auth image"
              />
              <NuxtImg
                v-else
                src="/images/noImageImage.jpg"
                class="w-8 h-8 rounded-full object-cover"
                alt="Auth image"
              />
              <NuxtLink
                :to="`/users/${props.bookDetails.renterId}`"
                class="text-sm cursor-pointer truncate w-45 font-semibold"
                >{{ props.bookDetails.renterUsername }}</NuxtLink
              >
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>

  <div v-else class="w-full h-[400px] flex flex-col items-center justify-center bg-transparent" />
</template>