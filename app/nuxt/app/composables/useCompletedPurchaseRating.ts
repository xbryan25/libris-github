...
export const useCompletedPurchaseRating = () => {
  const loading = ref(false)
  const error = ref<string | null>(null)

  const { $apiFetch } = useNuxtApp()

  const getCompletedPurchaseRatings = async (
    purchaseId: string
    ...
    try {
      ...
      { given_rating: number, 
        ...
        = await $apiFetch(`${API_URL}/api/ratings/purchase/${purchaseId}`, {
        credentials: 'include',
      })
